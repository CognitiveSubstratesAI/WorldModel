...
boundary_pressure(goals, mods) =
    (
        s=_eval1("(boundaryPressure $(_state(goals, mods)))");
        s === nothing ? nothing : parse(Float64, s)
    )
...
appraise(goals, mods, stimulus) =
    _parse_vec(
        _eval1(
            "(motivationModulators (openPsiAppraise $(_state(goals, mods)) (stimulus $(_vec(stimulus)))))"
        )
    )
...
function _magus_score(g, m, c, risk, dg)
    ...
    base - (0.5 * g[1] * caution * risk + conflict) +
    0.5 * g[2] * growth * (0.7 * max(0.0, explore) + 0.3 * gshift)
end
...
function govern(goals, mods, stimulus, candidates)
    am = _appraise_native(
        collect(Float64, goals), collect(Float64, mods), collect(Float64, stimulus)
    )  # Ψ, native
    ...
    for c in candidates                               # 𝔻 in native Julia — the 8.6s magusDecide → µs
        s = _magus_score(
            collect(Float64, goals),
            am,
            collect(Float64, c.corrs),
            float(c.risk),
            collect(Float64, c.dg)
        )
